...
{
    public class DatabaseRepository(ApplicationDbContext applicationDbContext, IUserContext userContext, TypeAdapterConfig typeAdapterConfig) : IDatabaseRepository
    {
        ...
        public async Task UpadteAsync<TEntity>(TEntity entity, CancellationToken cancellationToken = default)
            where TEntity : BaseEntity
        {
        ...
        public async Task UpadteAndSaveAsync<TEntity>(TEntity entity, CancellationToken cancellationToken = default)
            where TEntity : BaseEntity
        {
            await this.UpadteAsync(entity, cancellationToken);
            await this.SaveAsync(cancellationToken);
        }
        ...
        public async Task<TDestination?> GetFirstOrDefaultAsync<TEntity, TDestination>(Func<IQueryable<TEntity>, IQueryable<TEntity>> queryable, CancellationToken cancellationToken = default)
            ...
            where TDestination : class
        {
            return (await this.GetByConditionAsync<TEntity, TDestination>(queryable, cancellationToken)).FirstOrDefault();
        }
        ...
        public async Task<TEntity?> GetFirstOrDefaultAsync<TEntity>(Func<IQueryable<TEntity>, IQueryable<TEntity>> queryable, CancellationToken cancellationToken = default)
            where TEntity : BaseEntity
        {
            return (await this.GetByConditionAsync(queryable, cancellationToken)).FirstOrDefault();
        }
